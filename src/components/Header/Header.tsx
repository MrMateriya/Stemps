            ...
            <div className="app-container flex items-center justify-between py-4 pb-[15px] lg:grid grid-cols-[428px_1fr]">
                <div className="flex items-center justify-start space-x-4">
                    <Link href="/" className="w-7 h-7 border border-solid border-dark-gray rounded-[5px] bg-dark-gray flex items-center justify-center">
                        <svg className="w-[14px] h-[14px] fill-white" width="53" height="60" viewBox="0 0 53 60"
                    ...
                    <p className="hidden lg:block text-bl2 uppercase">STEMPS</p>
                </div>
                <div className="lg:flex lg:justify-between lg:items-center">
                    <ul className="hidden lg:flex space-x-6">
                        {links.map((item, index) => (
                          <li key={item.id}>
                              <Link className="text-bl2" href={item.href}>
                                  {item.title}
                              </Link>
                          </li>
                        ))}
                    </ul>
                    <Link href="#" className="hidden lg:flex text-bl2 gap-4">
                        Вход
